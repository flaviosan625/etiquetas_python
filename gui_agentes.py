...
_COR_NIVEL = {
    "ok": COR_POSITIVO,
    "atencao": COR_ALERTA,
    "parado": COR_RIP_PARADO,
    "sem_sinal": COR_TEXTO_SECUNDARIO,
    "rodando": COR_ACENTO,
    "congelado": COR_TEXTO_SECUNDARIO,
}
...
class JanelaAgentes(tk.Toplevel):
    ...
    def _montar_cabecalho(self):
        ...
        tk.Label(topo, text="O que roda sozinho, se está vivo, e o botão pra forçar quando não rodar.",
                 font=("Segoe UI", 9), bg=COR_FUNDO_JANELA, fg=COR_TEXTO_SECUNDARIO).pack(anchor="w")
        self.var_rodape = tk.StringVar(value="lendo...")
        tk.Label(topo, textvariable=self.var_rodape, font=("Segoe UI", 8),
                 bg=COR_FUNDO_JANELA, fg=COR_TEXTO_SECUNDARIO).pack(anchor="w", pady=(4, 0))
    ...
    def _montar_cartao(self, ag):
        ...
        simbolo = tk.Label(cartao, text="●", font=("Segoe UI", 16), bg=COR_CARTAO,
                           fg=COR_TEXTO_SECUNDARIO)
        simbolo.grid(row=0, column=0, rowspan=2, sticky="n", padx=(14, 8), pady=(10, 0))

        tk.Label(cartao, text=ag.nome, font=("Segoe UI", 11, "bold"), bg=COR_CARTAO,
                 fg=COR_TEXTO, anchor="w").grid(row=0, column=1, sticky="w", pady=(12, 0))
        tk.Label(cartao, text="%s  ·  %s" % (ag.faz, ag.onde), font=("Segoe UI", 9),
                 bg=COR_CARTAO, fg=COR_TEXTO_SECUNDARIO, anchor="w", justify="left",
        ...
        if ag.pode_disparar:
            btn_disparar = tk.Button(
                botoes, text="▶  Disparar agora", bg=COR_ACENTO, fg="white",
            ...
            linha_links = tk.Frame(botoes, bg=COR_CARTAO)
            linha_links.pack(anchor="e", pady=(4, 0))
            btn_aqui = tk.Button(
                linha_links, text="Rodar aqui e ver saída", relief="flat", bg=COR_CARTAO,
        ...
        tk.Button(linha_links, text="📂 Pasta", relief="flat", bg=COR_CARTAO, fg=COR_ACENTO,
        ...
        if ag.chave == "checklist":
            ...
            tk.Button(linha_links, text="👥 Clientes", relief="flat", bg=COR_CARTAO, fg=COR_ACENTO,
                      cursor="hand2", font=("Segoe UI", 8),
        ...
        lbl_estado = tk.Label(cartao, textvariable=var_estado, font=("Segoe UI", 10),
                              bg=COR_CARTAO, fg=COR_TEXTO, anchor="w", justify="left", wraplength=760)
        lbl_estado.grid(row=2, column=1, columnspan=2, sticky="w", padx=(0, 14), pady=(8, 0))

        var_acao = tk.StringVar(value="")
        tk.Label(cartao, textvariable=var_acao, font=("Segoe UI", 8), bg=COR_CARTAO,
                 fg=COR_TEXTO_SECUNDARIO, anchor="w", justify="left", wraplength=760,
                 ).grid(row=3, column=1, columnspan=2, sticky="w", padx=(0, 14), pady=(2, 0))

        motivo = _POR_QUE_SEM_BOTAO.get(ag.chave)
        if motivo:
            tk.Label(cartao, text=motivo, font=("Segoe UI", 8, "italic"), bg=COR_CARTAO,
                     fg=COR_TEXTO_SECUNDARIO, anchor="w").grid(
                row=4, column=1, columnspan=2, sticky="w", padx=(0, 14), pady=(2, 0))

        # a saída do "Rodar aqui" só aparece depois de usado
        frame_saida = tk.Frame(cartao, bg=COR_CARTAO)
        var_titulo_saida = tk.StringVar()
        tk.Label(frame_saida, textvariable=var_titulo_saida, font=("Segoe UI", 8, "bold"),
                 bg=COR_CARTAO, fg=COR_TEXTO, anchor="w").pack(anchor="w")
        texto_saida = tk.Text(frame_saida, height=7, bg="#0f1116", fg="#d6d9e0",
                              font=("Consolas", 9), relief="flat", wrap="word")
        texto_saida.pack(fill="x", pady=(2, 0))

        tk.Frame(cartao, bg=COR_CARTAO, height=12).grid(row=6, column=0, columnspan=3)
    ...
    def _atualizar(self):
        ...
        for chave, c in self._cartoes.items():
            ...
            cor = _COR_NIVEL.get(nivel, COR_TEXTO_SECUNDARIO)
            c["simbolo"].configure(text=_SIMBOLO_NIVEL.get(nivel, "●"), fg=cor)
            c["lbl_estado"].configure(fg=cor if nivel != "ok" else COR_TEXTO)
