...
def localizar_pastas_cliente(nome_cliente_seguro, pasta_saida_base="etiquetas_geradas"):
    """
    Lista as pastas já existentes desse cliente (mesmo nome sanitizado
    usado no prefixo "CLIENTE_timestamp"), mais recente primeiro pelo
    nome (o timestamp no nome já ordena assim). Cliente nunca
    processado antes: lista vazia.
    """
    base = pathlib.Path(pasta_saida_base)
    if not base.exists():
        return []
    prefixo = f"{nome_cliente_seguro.upper()}_"
    pastas = [p for p in base.iterdir() if p.is_dir() and p.name.upper().startswith(prefixo)]
